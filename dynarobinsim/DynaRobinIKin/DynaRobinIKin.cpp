...
DynaRobinIKin::DynaRobinIKin(){
	...
	FL_last_res=new vector<double>(3,0);
	FR_last_res=new vector<double>(3,0);
	BL_last_res=new vector<double>(3,0);
	BR_last_res=new vector<double>(3,0);
}
...
bool DynaRobinIKin::CalculateJacobianInverse(vector<double> FL,vector<double> FR, vector<double> BL, vector<double> BR, vector<double> ksi){
	...
	real_2d_array ksiVec;
	ksiVec.setlength(6,1);
    ksiVec(0,0) = ksi.at(0);
    ksiVec(1,0) = ksi.at(1);
    ksiVec(2,0) = ksi.at(2);
    ksiVec(3,0) = ksi.at(3);
	...
	///vracanje nazad
	FL_last_res->at(0) =qFL1+deltaQ(3,0)/FACTOR;
	//if(abs(FL_last_res->at(0))>PI/4) FL_last_res->at(0)=qFL1;
	FL_last_res->at(1) =qFL2+deltaQ(4,0)/FACTOR;
	//if(abs(FL_last_res->at(1))>PI/4) FL_last_res->at(1)=qFL2;
	FL_last_res->at(2) =qFL3+deltaQ(5,0)/FACTOR;
	//if(abs(FL_last_res->at(2))>PI/4) FL_last_res->at(2)=qFL3;

	FR_last_res->at(0) =qFR1+deltaQ(0,0)/FACTOR;
	//if(abs(FR_last_res->at(0))>PI/4) FR_last_res->at(0)=qFR1;
    FR_last_res->at(1) =qFR2+deltaQ(1,0)/FACTOR;
	//if(abs(FR_last_res->at(1))>PI/4) FR_last_res->at(1)=qFR2;
	FR_last_res->at(2) =qFR3+deltaQ(2,0)/FACTOR;
	//if(abs(FR_last_res->at(2))>PI/4) FR_last_res->at(2)=qFR3;

	BL_last_res->at(0) =qBL1+deltaQ(6,0)/FACTOR;
	//if(abs(BL_last_res->at(0))>PI/4) BL_last_res->at(0)=qBL1;
	BL_last_res->at(1) =qBL2+deltaQ(7,0)/FACTOR;
	//if(abs(BL_last_res->at(1))>PI/4) BL_last_res->at(1)=qBL2;
	BL_last_res->at(2) =qBL3+deltaQ(8,0)/FACTOR;
	//if(abs(BL_last_res->at(2))>PI/4) BL_last_res->at(2)=qBL3;

	BR_last_res->at(0) =qBR1+deltaQ(9,0)/FACTOR;
	//if(abs(BR_last_res->at(0))>PI/4) BR_last_res->at(0)=qBR1;
	BR_last_res->at(1) =qBR2+deltaQ(10,0)/FACTOR;
	//if(abs(BR_last_res->at(1))>PI/4) BR_last_res->at(1)=qBR2;
	BR_last_res->at(2) =qBR3+deltaQ(11,0)/FACTOR;
	//if(abs(BR_last_res->at(2))>PI/4) BR_last_res->at(2)=qBR3;
...
bool DynaRobinIKin::CalculateInverseKinematics(vector<double> FootPosition, double KneeSide, vector<double> *JointPosition){
	double K1, K2, K3, z0;
	double q123, q1, q2, q3, q0;

	FootPosition.at(0) = FootPosition.at(0) - l4;

	static double PreviousX = 0;
	static double PreviousY = 0;
	static double PreviousZ = -0.20;

	// z0 postaje z iz perspektive stare noge
	// iz y raèunam zakret u ramenu
	q0 = atan(FootPosition.at(1) / FootPosition.at(2));
	z0 = FootPosition.at(2) / cos(q0);

	K1 = FootPosition.at(0);
	K2 = z0;
	q123 = 0;
	if (((l5 * l5 + 2 * l5 * l6 + l6 * l6 - K1 * K1 - K2 * K2)
		/ (-l5 * l5 + 2 * l5 * l6 - l6 * l6 + K1 * K1
		+ K2 * K2)) >= 0)
	{
		K1 = FootPosition.at(0);
		K2 = z0;

		PreviousX = FootPosition.at(0);
		PreviousY = FootPosition.at(1);
		PreviousZ = z0;
	}
	else {
		K1 = PreviousX;
	...
	K3 = sqrt(
		(l5 * l5 + 2 * l5 * l6 + l6 * l6 - K1 * K1 - K2 * K2)
		/ (-l5 * l5 + 2 * l5 * l6 - l6 * l6 + K1 * K1
		+ K2 * K2));
	...
	q1 = (-2// promijenio sam predznak q1
		* atan(
		(2 * l5 * K1
		- KneeSide
		* (l5 * l5 + l6 * l6 - K1 * K1 - K2 * K2
		- 2 * l5 * l6) * K3)
		/ (l5 * l5 - 2 * l5 * K2 - l6 * l6 + K1 * K1
		+ K2 * K2)));
	q2 = (KneeSide * 2 * atan(K3));



	JointPosition->at(0) = q0;
	JointPosition->at(1) = q1;
	JointPosition->at(2) = q2;
	return true;
}


bool DynaRobinIKin::CalculateDirectKinematics(vector<double> JointPosition, vector<double> *FootPosition){


	//double a2 = 100.2, a3 = 204.25, a4 = 19.08, d2 = 53;
	...
	//x = d2*sin(q1) + a2*cos(q1)*cos(q2) + a4*cos(q2 + q3)*cos(q1)*cos(q4) - a4*sin(q2 + q3)*cos(q1)*sin(q4) + a3*cos(q1)*cos(q2)*cos(q3) - a3*cos(q1)*sin(q2)*sin(q3);
	//y = a2*cos(q2)*sin(q1) - d2*cos(q1) + a4*cos(q2 + q3)*cos(q4)*sin(q1) - a4*sin(q2 + q3)*sin(q1)*sin(q4) + a3*cos(q2)*cos(q3)*sin(q1) - a3*sin(q1)*sin(q2)*sin(q3);
	//z = a3*sin(q2 + q3) + a2*sin(q2) + a4*sin(q2 + q3 + q4);
	...
	double ld6 = 0.12;
	double Proj_z = l5*cos(JointPosition.at(1)) + l6*cos(JointPosition.at(1) + JointPosition.at(2)); //projekcija noge na z os

	double fi = (PI - (JointPosition.at(0))) / 2;
	double d = 2 * Proj_z*sin(JointPosition.at(0) / 2);
	double dz = d*cos(fi);

	FootPosition->at(0) = -l5*sin(JointPosition.at(1)) - l6*sin(JointPosition.at(1) + JointPosition.at(2));
	FootPosition->at(1) = -sin(JointPosition.at(0))*Proj_z;		//pozitivan zakret motora pomièe nogu u negativnom smjeru y osi
	FootPosition->at(2) = dz - Proj_z;
